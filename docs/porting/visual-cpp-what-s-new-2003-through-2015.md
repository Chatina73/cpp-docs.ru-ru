...
- **Параметры шаблона, не являющиеся типами**
   ...
   Текущий компилятор правильно выдает ошибку, так как тип параметра шаблона не соответствует аргументу шаблона (параметр является указателем на элемент const, а функция f не является константой):
...
- **Перегруженный оператор new и оператор delete**
   ...
   Кроме того, несмотря на то, что компилятор не предоставляет конкретную диагностику, встроенный оператор New считается некорректным.
...
- * * Удаление `pow(T, int)` оптимизации для развертывания * *

   В предыдущих версиях стандартной библиотеки C++ `pow(T, int)` был определен шаблон функции, который бы мог отменить `pow` вызов функции в ряд операций умножения. Этот метод начисляется большим количеством неточностей из-за природы операций с плавающей запятой, что приводит к значительному неточности результатов. В Visual Studio 2015 с обновлением 1 это поведение было удалено во избежание непреднамеренной потери точности при использовании `pow` функции. Однако эта версия `pow` выполнялась гораздо быстрее, чем правильное вычисление. Если это изменение вызывает значительную регрессию производительности и проект не требует точных результатов с плавающей запятой (например, проект уже компилируется с/FP: Fast), рекомендуется заменить вызовы на `pow` с помощью этой функции обхода.
   ...
   Эта реализация идентична той, которая была включена в предыдущие версии Visual Studio.
