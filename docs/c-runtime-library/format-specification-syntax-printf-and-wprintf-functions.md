...
ms.locfileid: "107539553"
---
# <a name="format-specification-syntax-printf-and-wprintf-functions"></a>Синтаксис спецификации формата: `printf` и `wprintf` функции
...
В спецификации преобразования необязательное поле спецификации ширины отображается после любых символов *флага*. *`width`* Аргумент представляет собой Неотрицательное десятичное целое число, которое управляет минимальным числом выводимых символов. Если число символов в выходном значении меньше указанной ширины, то слева или справа от значений добавляются пробелы, в зависимости от того, задан ли флаг выравнивания влево ( **`-`** ), до тех пор, пока не будет достигнута минимальная ширина. Если параметр *`width`* имеет префикс 0, ведущие нули добавляются к целому числу или преобразованиям с плавающей запятой до тех пор, пока не будет достигнута минимальная ширина, за исключением случаев, когда преобразование выполняется в бесконечное значение или `NaN` .
...
Если спецификация точности представляет собой звездочку (`*`), аргумент `int` из списка аргументов предоставляет значение. В списке аргументов *`precision`* аргумент должен предшествовать отформатированному значению, как показано в следующем примере:

`printf( "%.*f", 3, 3.14159265 );  /* 3.142 output */`

*`type`* Символ определяет либо интерпретацию, *`precision`* либо точность по умолчанию *`precision`* , если аргумент не указан, как показано в следующей таблице.
