...
ms.locfileid: "107539795"
---
# <a name="fopen_s-_wfopen_s"></a>`fopen_s`, `_wfopen_s`

Открывает файл. Эти [ `fopen` `_wfopen` версии имеют улучшения безопасности, как](fopen-wfopen.md) описано в разделе [функции безопасности в CRT](../../c-runtime-library/security-features-in-the-crt.md).
...
Возвращает нуль в случае успеха или код ошибки в случае неудачи. Дополнительные сведения об этих кодах ошибок см. в статьях [ `errno` ,, `_doserrno` `_sys_errlist` и `_sys_nerr` ](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md).
...
Файлы, открытые **`fopen_s`** и **`_wfopen_s`** не являющиеся совместно используемыми. Если требуется, чтобы файл был совместно используемым, используйте [ `_fsopen` , `_wfsopen`](fsopen-wfsopen.md) с соответствующей константой режима общего доступа, например **`_SH_DENYNO`** для общего доступа на чтение и запись.
...
В режиме текста (преобразованном) **CTRL** + **Z** интерпретируется как символ конца файла во входных данных. В файлах, открытых для чтения и записи с **`"a+"`** , **`fopen_s`** проверяет наличие **CTRL** + **Z** в конце файла и удаляет его, если это возможно. Это делается потому, что использование [`fseek`](fseek-fseeki64.md) и **`ftell`** для перемещения в файле, заканчивающемся **CTRL** + **Z**, может вызвать [`fseek`](fseek-fseeki64.md) неправильное поведение в конце файла.
