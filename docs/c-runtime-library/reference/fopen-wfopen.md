...
ms.locfileid: "107539521"
---
# <a name="fopen-_wfopen"></a>`fopen`, `_wfopen`

Открывает файл. Доступны более безопасные версии этих функций, которые выполняют дополнительные проверки параметров и возвращают коды ошибок. см. раздел [ `fopen_s` , `_wfopen_s` ](fopen-s-wfopen-s.md).
...
Дополнительные сведения см. в статьях [ `errno` ,, `_doserrno` `_sys_errlist` и `_sys_nerr` ](../../c-runtime-library/errno-doserrno-sys-errlist-and-sys-nerr.md).

## <a name="remarks"></a>Комментарии

**`fopen`** Функция открывает файл, указанный параметром *filename*. По умолчанию строка с узким *именем* интерпретируется с помощью кодовой страницы ANSI ( `CP_ACP` ). В классических приложениях Windows это можно изменить на кодовую страницу OEM ( `CP_OEMCP` ) с помощью [`SetFileApisToOEM`](/windows/win32/api/fileapi/nf-fileapi-setfileapistooem) функции. С помощью функции можно [`AreFileApisANSI`](/windows/win32/api/fileapi/nf-fileapi-arefileapisansi) определить, интерпретируется ли *имя файла* с помощью ANSI или системной кодовой страницы OEM по умолчанию. **`_wfopen`** — Это версия с расширенными символами **`fopen`** ; аргументы для **`_wfopen`** представляют собой строки расширенных символов. В противном случае **`_wfopen`** **`fopen`** поведение идентично. Только использование **`_wfopen`** не влияет на закодированную кодировку, используемую в файловом потоке.
