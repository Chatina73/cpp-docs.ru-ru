...
*`lower`*\
Нижняя граница значений, до которой производится фиксация *`value`* .

*`upper`*\
Верхняя граница значений, в которую производится фиксация *`value`* .

*`pred`*\
Предикат, используемый для сравнения *`value`* с *`lower`* или *`upper`* . Предикат сравнения принимает два аргумента и возвращает **`true`** , если первый имеет смысл меньше второго, и в противном случае — **`false`** .

### <a name="return-value"></a>Возвращаемое значение

Возвращает ссылку на *`lower`* if `value < lower` или ссылку на *`upper`* if `upper < value` . В противном случае возвращается ссылка на *`value`* .
...
Итератор вывода, обращающийся к позиции, которая находится за последним элементом в диапазоне назначения, то есть итератор обращается к `result` + ( *`last`*  -  *`first`* ).
...
Итератор вывода, обращающийся к позиции, которая находится за последним элементом в диапазоне назначения, то есть итератор обращается к *`destEnd`* ( *`last`*  -  *`first`* ).
...
Выходной итератор, *`dest`* увеличивающийся один раз для каждого элемента, удовлетворяющего условию. Иными словами, возвращаемое значение минус *`dest`* равно количеству скопированных элементов.
...
один раз для каждого `N` в диапазоне `[0, last - first)` строго на увеличение значений `N`, начиная с наименьшего значения. Если *`dest`* и *`first`* обозначают области хранилища, *`dest`* не должен находиться в диапазоне `[ first, last )` .
...
Функция шаблона проверяет `*(dest + N) = *(first + N))` один раз для каждого `N` в диапазоне `[0, count)`строго на увеличение значений `N` начиная с наименьшего значения. Затем оно возвращает значение `dest + N`. Если *`dest`* и *`first`* обозначают области хранилища, *`dest`* не должен находиться в диапазоне `[first, last)` .
...
Тип разницы объекта `InputIterator` , который подсчитывает количество элементов в диапазоне [ *`first`* , *`last`* ) со значением *`value`* .
...
Этот алгоритм обобщен для подсчета элементов, которые соответствуют любому предикату с помощью функции-шаблона [`count_if`](algorithm-functions.md#count_if) .
...
Эта функция шаблона представляет собой обобщение алгоритма [`count`](algorithm-functions.md#count) , заменяя предикат "равно определенному значению" любым предикатом.
...
Пара прямых итераторов, указывающих поддиапазон, содержащийся в диапазоне, в котором все элементы эквивалентны *`value`* в смысле, определяемом используемым бинарным предикатом ( *`pred`* или по умолчанию — "меньше чем").

Если ни один элемент в диапазоне не эквивалентен *`value`* , то прямые итераторы в возвращаемой паре равны и указывают точку, где *`value`* можно вставить, не нарушая порядок диапазона.

### <a name="remarks"></a>Комментарии

Первым итератором пары, возвращаемым алгоритмом, является [`lower_bound`](algorithm-functions.md#lower_bound) , а вторым итератором является [`upper_bound`](algorithm-functions.md#upper_bound) .
...
*`first`*\
Выходной итератор, обращающийся к положению первого элемента в диапазоне, которому присваивается значение *`value`* .
...
Итератор на элемент, следующий за последним элементом, который был заполнен *`count`* , если > ноль, в противном случае — первый элемент.
...
Входной итератор, адресующий первое вхождение заданного значения в диапазоне, в котором выполняется поиск. Если элемент с эквивалентным значением не найден, возвращается значение *`last`* .

### <a name="remarks"></a>Комментарии

`operator==`, используемый для определения совпадения между элементом и указанным значением, должен применять отношение эквивалентности между своими операндами.

Пример кода с помощью см `find()` . в разделе [`find_if`](algorithm-functions.md#find_if) .
...
*`pred`*\
Определенный пользователем объект функции предиката или [лямбда-выражение](../cpp/lambda-expressions-in-cpp.md), определяющее условие, которому должен соответствовать искомый элемент. Унарный предикат принимает один аргумент и возвращает результат **`true`** , если он удовлетворен, или значение, **`false`** если оно не удовлетворено. Сигнатура *`pred`* должна быть эффективной `bool pred(const T& arg);` , где `T` — это тип, который `InputIterator` может быть неявно преобразован при разыменовании. **`const`** Ключевое слово отображается только для иллюстрации того, что объект функции или лямбда-выражение не должны изменять аргумент.

### <a name="return-value"></a>Возвращаемое значение

Входной итератор, который ссылается на первый элемент в диапазоне, удовлетворяющий условию, заданному предикатом (результат предиката **`true`** ). Если элемент не найден для удовлетворения предиката, функция возвращает значение *`last`* .

### <a name="remarks"></a>Комментарии

Эта функция шаблона представляет собой обобщение алгоритма [`find`](algorithm-functions.md#find) , заменяя предикат "равно определенному значению" любым предикатом. Для логического противоположного (поиск первого элемента, не удовлетворяющего предикату) см. в разделе [`find_if_not`](algorithm-functions.md#find_if_not) .
...
*`pred`*\
Определенный пользователем объект функции предиката или [лямбда-выражение](../cpp/lambda-expressions-in-cpp.md), определяющее условие, которому не должен соответствовать искомый элемент. Унарный предикат принимает один аргумент и возвращает результат **`true`** , если он удовлетворен, или значение, **`false`** если оно не удовлетворено. Сигнатура *`pred`* должна быть эффективной `bool pred(const T& arg);` , где `T` — это тип, который `InputIterator` может быть неявно преобразован при разыменовании. **`const`** Ключевое слово отображается только для иллюстрации того, что объект функции или лямбда-выражение не должны изменять аргумент.

### <a name="return-value"></a>Возвращаемое значение

Входной итератор, ссылающийся на первый элемент в диапазоне, который не удовлетворяет условию, заданному предикатом (результат предиката **`false`** ). Если все элементы соответствуют предикату (предикат приводится **`true`** к каждому элементу), возвращает *`last`* .

### <a name="remarks"></a>Комментарии

Эта функция шаблона представляет собой обобщение алгоритма [`find`](algorithm-functions.md#find) , заменяя предикат "равно определенному значению" любым предикатом. Для логического противоположного (поиск первого элемента, удовлетворяющего предикату) см. в разделе [`find_if`](algorithm-functions.md#find_if) .

Пример кода, который легко адаптируется к `find_if_not()` , см. в разделе [`find_if`](algorithm-functions.md#find_if) .
...
Сложность линейная, но не более ( *`last`*  -  *`first`* ) сравнений.
...
Сложность зависит от объема доступной памяти, так как алгоритм выделяет память для временного буфера. Если доступен достаточный объем памяти, лучший вариант — линейный с `(last - first) - 1` сравнениями; Если вспомогательная память недоступна, в худшем случае — `N log(N)` , где *`N`*  =  *`last`*  -  *`first`* .
...
Возвращает итератор, расположенный на первом элементе диапазона [ `first` , `last` ), который не удовлетворяет условию упорядочивания кучи, или *`end`* значение, если диапазон образует кучу.
...
Первая функция шаблона возвращает последний итератор `next` в `[first, last)`, где `[first, next)` — это куча, упорядоченная по объекту функции `std::less<>`. Если расстояние `last - first` меньше 2, функция возвращает значение *`last`* .

Вторая функция шаблона ведет себя так же, как первая, за исключением того, что в *`pred`* `std::less<>` качестве условия упорядочивания кучи используется предикат, а не.
...
Вторая функция шаблона возвращает `is_sorted_until( first, last , pred ) == last`. *`pred`* Функция предиката выполняет сравнение порядка.
...
Возвращает `ForwardIterator`, установленный в последний элемент в порядке сортировки. Упорядоченная последовательность начинается с *`first`* .

### <a name="remarks"></a>Комментарии

Первая функция-шаблон возвращает последний итератор `next` в `[first, last]`, так что `[first, next)` — это последовательность, упорядоченная по `operator<`. Если `distance()` значение меньше 2, функция возвращает значение *`last`* .
...
При использовании алгоритма `max` очень редко объекты передаются как параметры. Большинство алгоритмов библиотеки стандартных программ C++ работают в диапазоне элементов, позиция которых задается итераторами, передаваемыми в качестве параметров. Если требуется функция, которая работает с диапазоном элементов, используйте [`max_element`](algorithm-functions.md#max_element) вместо. Visual Studio 2017 включает **`constexpr`** в себя перегрузки, которые принимают `initializer_list` .
...
[ `list` Класс](list-class.md) предоставляет функцию-член `merge` для слияния элементов двух списков.
...
При использовании алгоритма `min` очень редко объекты передаются как параметры. Большинство алгоритмов библиотеки стандартных программ C++ работают в диапазоне элементов, позиция которых задается итераторами, передаваемыми в качестве параметров. Если требуется функция, использующая диапазон элементов, используйте [`min_element`](algorithm-functions.md#min_element) . [`constexpr`](../cpp/constexpr-cpp.md) был включен для `initializer_list` перегрузок в Visual Studio 2017.
...
Первая функция-шаблон возвращает `pair<const Type&, const Type&>( right, left )` значение *`right`* , если меньше *`left`* . В противном случае возвращается значение `pair<const Type&, const Type&>( left, right )`.

Вторая функция-член возвращает пару, в которой первый элемент является меньшим, а второй — более поздним по сравнению с предикатом *`pred`* .

Остальные функции шаблона ведут себя одинаково, за исключением того, что они заменяют *`left`* *`right`* Параметры и на *`inlist`* .
...
Функция шаблона проверяет `*(dest + N) = move(*(first + N))` один раз для каждого `N` в диапазоне `[0, last - first)`строго на увеличение значений `N` начиная с наименьшего значения. Затем оно возвращает значение `dest + N`. Если `dest` и *`first`* обозначают области хранилища, *`dest`* не должен находиться в диапазоне `[first, last)` .
...
Среднее значение сложности сортировки линейное по отношению к *`last - first`* .
...
Сложность линейная: существуют `(last - first)` приложения *`pred`* и, в большинстве случаев, `(last - first)/2` подкачки.
...
Функция шаблона находит первый итератор `it` в `[first, last)` , который `pred(*it)` имеет значение **`false`** . Последовательность должна быть упорядочена по *`pred`* .
...
`std::random_shuffle()`Функция является устаревшей, она заменена на [`std::shuffle`](algorithm-functions.md#shuffle) . Пример кода и дополнительные сведения см [`<random>`](random.md) . в статьях и Stack overflow POST, [почему `std::random_shuffle` методы являются устаревшими в c++ 14?](https://go.microsoft.com/fwlink/p/?linkid=397954).
...
[ `list` Класс](list-class.md) имеет более эффективную версию функции-члена `remove` , которая также повторно связывает указатели.
...
*`searcher`*\
Поиск, инкапсулирующий искомый шаблон и используемый алгоритм поиска. Дополнительные сведения о поисковых запросах см. в разделе [ `default_searcher` класс](default-searcher-class.md), [ `boyer_moore_horspool_searcher` класс](boyer-moore-horspool-searcher-class.md)и [ `boyer_moore_searcher` класс](boyer-moore-searcher-class.md).
...
Кучи — это идеальный способ реализации очередей с приоритетами и их использования в реализации [ `priority_queue` класса](priority-queue-class.md)адаптера контейнера стандартной библиотеки C++.
...
Если *результат* установлен равным *`first1`* в первой версии алгоритма, исходный и конечный диапазоны будут одинаковыми, а последовательность будет изменена на месте. Но *`result`* может не обращаться к положению в диапазоне [ `first1` + 1, `last1` ).
