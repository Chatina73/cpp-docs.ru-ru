...
В приведенном выше примере первое представление действует как итератор, который предоставляет только элементы `input` , которые делятся на три. Другое представление выступает в качестве итератора, который принимает элементы, кратные трем, и предоставляет квадрат элемента.
...
Алгоритмы диапазона являются отложенными, то есть они работают с диапазоном только при обращении к элементу. Они могут работать непосредственно с контейнером. их можно легко объединить в цепочку.

## <a name="types-of-ranges"></a>Типы диапазонов

Действия, которые можно выполнять с диапазоном, зависят от типа базового итератора диапазона. Существуют различные типы диапазонов, называемые уточнениями. Различные типы диапазонов кодифицированные как основные понятия C++ 20. В этой таблице перечислены различные основные понятия диапазона, а также тип контейнера, к которому они могут применяться.

| Уточнение диапазона | Описание | Поддерживаемые контейнеры |
|--|--|--|
